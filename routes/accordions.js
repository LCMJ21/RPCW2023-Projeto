...
const descriptor = require('../controllers/descriptor');
...
router.get('/', verificaAcesso, createFilter, async function (req, res, next) {
    try {
        ...
        const perPage = 20;
        const labels = await descriptor.getDescriptors();
        const accordions_list = await accordions.list(
            req.oldquerry.page,
            perPage,
            req.filter
        );
        res.cookie(
            'previousUrl',
            '/?page=' + req.oldquerry.page + '&' + req.oldurl
        ).render('homepage', {
            title: 'Justiça Home',
    ...
    try {
        ...
        if (!accordion) {
            res.status(404).render('error', {
                error: 'Accordion' + id + 'not found',
            });
            return;
        }
        res.cookie('previousUrl', '/accordion/' + req.params.id).render(
            'accordion',
            {
